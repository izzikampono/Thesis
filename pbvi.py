...
class PBVI:
    ...
    def backward_induction(self):
        ...
        for timestep in range(self.horizon-1,-1,-1):
            ...
            # for alpha,belief in zip(self.value_function.vector_sets[timestep],self.belief_space.time_index_table[timestep]):
            #     print(f"belief = {belief} , alpha = {alpha.vectors}")
            # ## check solutions
            # for belief_id in self.belief_space.time_index_table[timestep]:
    ...
    def solve_sampled_densities(self,iterations,min_density):
        ...
        times = []
        belief_sizes = []
        densities = Utilities.exponential_decrease(1/len(CONSTANT.STATES), min_density,iterations)
