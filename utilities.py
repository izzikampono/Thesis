...
# #using DRs
# def  Pr_Z(z,b,aj):
#     """function to calculate prob of an observation given a belief and joint DR aj"""

#     sum=0
#     for x0 in states:
#         for x1 in states:
#             for u in prob.joint_actions:
#                     sum += b[x0] * aj[u] * prob.transition_fn[u][x0][x1] * prob.observation_fn[u][x0][z]
#     return sum
...
#     """ returns value of belief at timestep t  in the bag of beliefs """
#     max = np.inf
#     beliefs_t = V_table[t].keys()
#     random.sample(beliefs_t, len(beliefs_t))
#     for bt_next in beliefs_t:
#         distance = np.abs(np.linalg.norm(np.array(bt) - np.array(bt_next)))
#         if distance<max: 
#             next = tuple(bt_next)
#             max = distance
#     try: 
#         if next:
#             return V_table[t][next][0],V_table[t][next][1],V_table[t][next][2],V_table[t][next][3]
#     except:
...
def LP(Q1,Q2):
    ...
    return milp.solution.get_objective_value(),milp.solution.get_values(player1_DR),milp.solution.get_values(player2_DR),milp.solution.get_values(joint_DR)

def get_joint_DR(DR0,DR1):
    DR=np.zeros(len(joint_actions))
    idx = 0
    for action_leader in actions[0]:
        for follower_leader in actions[1]:
            DR[idx]=DR0[action_leader] * DR1[follower_leader]
            idx+=1
    return DR
...
def LP_zerosum2(payoff):
    ...
    for agent in range(2):
        ...
        for action in actions[agent]:
            DR[agent].append(milp.continuous_var(name=f"a{agent}_{action}",ub=1,lb=0))
    ...
    for agent in range(2):
        for other_agent_action in actions[int(not agent)]:    
            rhs = 0       
            value = 0
            for agent_action,agent_action_probability in enumerate(DR[agent]):
                # get joint action 
                if agent==1:
                    joint_action = problem.get_joint_action(other_agent_action,agent_action)
                else:
                    joint_action = problem.get_joint_action(agent_action,other_agent_action)
...
def LP_zerosum(agent,payoff):
    ...
    for other_agent_action in actions[int(not agent)]:    
        ...
        for agent_action,agent_action_probability in enumerate(DR):
            ...
            rhs += payoff[joint_action] * agent_action_probability
        milp.add_constraint(V[other_agent_action]<=rhs)


    #add sum-to-one constraint
    value = 0
    for action_probability in DR:
        value+=action_probability
    milp.add_constraint(value == 1)

    #solve and export 
    sol = milp.solve()
    milp.export_as_lp(f"SOTA_zerosum_tigerProblem_seperate_LP_{agent}")
        ...
        for agent in range(0,2):
            ...
            for hidden_state in states:
                for joint_action in joint_actions:
                    ...
                    if self.sota==True and agent==1 :continue #for blind agent
    ...
    def get_alpha_vector(self,DR,sota2=False):
        ...
        for x in states:
            ...
            for u in joint_actions:
                vectors[0][x] += DR.joint[u] * self.two_d_vectors[0][x][u]
                vectors[1][x] += DR.joint[u] * self.two_d_vectors[1][x][u]
