...
class Utilities:
    ...
    def  observation_probability(self,joint_observation,belief,joint_action):
        ...
        sum=0
        for state in self.constant.STATES:
            for next_state in self.constant.STATES:
                    sum += belief.value[state]  * self.constant.TRANSITION_FUNCTION[joint_action][state][next_state] * self.constant.OBSERVATION_FUNCTION[joint_action][state][joint_observation]
        return sum
    ...
    def LP(self,Q1,Q2):
        ...
        milp = Model(f"{self.constant.PROBLEM.name} problem")
        leader_DR = milp.continuous_var_list(len(self.constant.ACTIONS[0]),name = [f"a0_{i}" for i in self.constant.ACTIONS[1]],ub=1,lb=0)
        follower_DR = milp.binary_var_list(len(self.constant.ACTIONS[1]), name = [f"a1_{i}" for i in self.constant.ACTIONS[0]])
        joint_DR = milp.continuous_var_list(len(self.constant.JOINT_ACTIONS),name  = [f"aj_{i}" for i in self.constant.JOINT_ACTIONS],ub=1,lb=0)
        ...
        for follower_action in self.constant.ACTIONS[1]:    
            rhs = 0       
        ...
        joint_sum = 0
        for leader_action in self.constant.ACTIONS[0]:
            value = 0
            for follower_action in self.constant.ACTIONS[1]:   
                joint_action = self.PROBLEM.get_joint_action(leader_action,follower_action)
        ...
        for follower_action in self.constant.ACTIONS[0]:
            value = 0
            for leader_action in self.constant.ACTIONS[1]:   
                joint_action = self.PROBLEM.get_joint_action(leader_action,follower_action)
                value+=joint_DR[joint_action]
    ...
    def get_joint_DR(self,DR0,DR1):
        DR=np.zeros(len(self.constant.JOINT_ACTIONS))
        for leader_action in self.constant.ACTIONS[0]:
            for follower_action in self.constant.ACTIONS[1]:
    ...
    def sota_strategy(self,P1,P2, game_type):
        ...
        if game_type=="cooperative":
            ...
            for joint_action in self.constant.JOINT_ACTIONS:
                if max<P1[joint_action]:
                    max = P1[joint_action]
                    optimal_joint_action = joint_action
            DR_joint =  np.identity(len(self.constant.JOINT_ACTIONS))[optimal_joint_action]
            action_0, action_1 = self.PROBLEM.get_seperate_action(optimal_joint_action)
            DR0 = np.identity(len(self.constant.ACTIONS[0]))[action_0]
            DR1 = np.identity(len(self.constant.ACTIONS[1]))[action_1]
            return max,DR_joint,DR0,DR1


    def zerosum_lp_leader(self,payoff):
        "linear program for SOTA of zerosum game"
        lp = Model(f"{self.PROBLEM} problem")

        #initialize linear program variables
        DR =  lp.continuous_var_list(len(self.constant.ACTIONS[0]),name = [f"a0_{i}" for i in self.constant.ACTIONS[1]],ub=1,lb=0)
        ...
        # define constraints 
        for opponent_action in self.constant.ACTIONS[1]:    
            rhs = 0   
    ...
    def zerosum_lp_follower(self,payoff):
        ...
        DR = []
        V = []
        for action in self.constant.ACTIONS[1]:
        ...
        # define constraints 
        for opponent_action in self.constant.ACTIONS[0]:    
            rhs = 0   
